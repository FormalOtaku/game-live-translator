...
### CHG-20260528-030
- Date: 2026-05-28
- Summary: Added the dependency-free desktop UI shell route/state contract for T-011-001.
- Reason: The future Electron/React renderer needs one executable contract for entry-route resolution, the first-class screen registry, sanitized `AppStatus` consumption, localhost overlay URL trust, and recovery-action derivation before any concrete React components or Electron host code lands.
- Impact scope (DB/API/UI):
  - DB: No schema change. The shell is in-process and stateless beyond its in-memory view-model snapshot; it adds no SQLite tables, no profile export schema bump, no key storage movement, no cache persistence, no logs/diagnostics persistence, and no screenshot/image persistence.
  - API: Adds `src/ui/desktop-shell.js` with `ROUTE_REGISTRY`, `SIDEBAR_ROUTES`, `isSetupComplete`, `resolveEntryRoute`, `normalizeRoute`, `isTrustedOverlayUrl`, `sanitizeRuntimeStatus`, `sanitizeLastSubtitle`, `sanitizeAppStatus`, `deriveRecoveryActions`, `buildViewModel`, and `createDesktopShell`. The module has no runtime dependency on `createLocalApiServer`, adds no npm dependencies, and does not modify existing API routes or contracts.
  - UI: First-Run vs Home entry routing, persistent sidebar grouping, recovery copy derivation, and overlay URL safety now have an executable boundary that locks the UI_SPEC.md invariants (every first-class screen declares loading/empty/error/success/recovery; recovery actions are driven by `RuntimeStatus.code`/`retryable` only; `lastSubtitle` exposes only `escapedText`; only backend-reported `http://127.0.0.1:<port>/overlay` is trusted).
- Risk: This is the dependency-free contract module only; concrete React components and the Electron host process are not in this slice. New `RuntimeStatus.code` or `ApiError.code` values added by future backend slices must be added to `RECOVERY_ACTIONS_BY_CODE` in the same slice so the renderer does not silently fall back to `open_diagnostics`.
- Regression tests added first: `test/desktop-shell.test.js` covers route registry completeness/capabilities, sidebar ordering, setup-complete gating, entry-route resolution, unknown-route fallback (including prototype-pollution-shaped strings), overlay URL trust (port mismatch, LAN/localhost rejection, query/fragment/userinfo rejection, HTTPS/non-string rejection, optional-port behavior), runtime-status sanitization (message/provider key/sourceText drop), `lastSubtitle` sanitization (sourceText/translatedText/provider drop), AppStatus sanitization with overlay URL distrust, recovery action derivation from code/retryable (idle/ok/running -> empty, warning/error known/retryable/unknown codes, ignore message text), view-model gating (incomplete setup-required routes force first-run; complete setup honors requested route; unknown route falls back to home), and the stateful `createDesktopShell` harness (navigate, consumeAppStatus, updateSetup allow-listing setup fields, setPort distrusting cached URLs).
- Migration needed: None.
- Rollback plan: Remove `src/ui/desktop-shell.js`, `test/desktop-shell.test.js`, and the T-011-001 spec/decision entries. Existing PRODUCT_SPEC.md/UI_SPEC.md/API_SPEC.md/DECISIONS.md/CHANGELOG_DECISIONS.md/MIGRATION_PLAN.md entries for prior tasks remain unchanged.

### CHG-20260528-031
- Date: 2026-05-28
- Summary: Added the dependency-free First-Run provider/profile setup flow contract for T-011-002.
- Reason: The future Electron/React First-Run Wizard needs one executable contract for provider readiness, profile creation, OCR/translation test sequencing, active-profile finish, and sensitive-input redaction before concrete components are built.
- Impact scope (DB/API/UI):
  - DB: No schema change. The flow is a pure in-process renderer contract and adds no SQLite tables, no profile export schema bump, no provider key storage movement, no cache persistence, no logs/diagnostics persistence, and no screenshot/image persistence.
  - API: Adds `src/ui/first-run-flow.js` with frozen step definitions, allow-listed profile draft state, HTTP intent builders for `GET /api/capture/sources`, `POST /api/profiles`, `PUT /api/keys/{provider}`, `POST /api/ocr/test`, `POST /api/translate/test`, and `PUT /api/profiles/active`, plus validation through existing contract validators. No server routes are added or changed.
  - UI: First-Run now has an executable flow state and view-model boundary. The internal `persist-profile` step makes the existing profile-bound OCR/translation test APIs explicit; `echo` is keyless debug-ready; `deepl` is write-only-key gated; `deriveDesktopSetup(state)` feeds the `desktop-shell` setup gate.
- Risk: This remains a renderer contract, not concrete React UI. Components must preserve the contract by keeping provider key and manual test text outside global state/logs and by clearing fetch-time inputs after submission. A future backend draft-profile route would require a coordinated API/UI/spec update.
