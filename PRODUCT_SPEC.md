...
- T-011-001 adds a dependency-free desktop UI shell route/state contract (`src/ui/desktop-shell.js`) for the future Electron/React renderer: pure JS, no persistence, no SQLite schema change, and no runtime dependency on `createLocalApiServer`. It registers all first-class screens with loading/empty/error/success/recovery capabilities, picks the First-Run wizard vs Home entry route from explicit setup completeness, sanitizes `AppStatus`/`RuntimeStatus`/`SubtitleFrame` consumption to drop `sourceText`, `translatedText`, provider keys, and raw runtime messages, trusts only backend-reported `http://127.0.0.1:<port>/overlay` URLs, and derives recovery actions from `RuntimeStatus.code`/`retryable` without parsing message text.
- T-011-002 adds a dependency-free First-Run flow contract (`src/ui/first-run-flow.js`) for the future Electron/React renderer. It models the production setup sequence from boundary/privacy acknowledgement through provider readiness, capture source, ROI, profile creation, OCR/translation tests, and active-profile finish while emitting only HTTP intent descriptors for the existing localhost API. API keys and raw translation test text stay outside serializable state and log-safe descriptors; OCR/translation results update booleans only; `deriveDesktopSetup(state)` feeds the `desktop-shell` setup gate without duplicating route logic.
- T-011-003 adds a dependency-free Home/Status action contract (`src/ui/home-status-actions.js`) for the future Electron/React renderer. It turns the sanitized desktop-shell view model into streamer-ready readiness cards and safe action intents for `/api/status`, `/ws/app`, trusted overlay-URL clipboard copy, OBS setup navigation, capture start/stop, backend restart, diagnostics, and relevant settings routes. It never constructs overlay URLs from user or LAN hosts, never parses runtime messages, and keeps provider keys, OCR/source text, translated text, stack traces, screenshots, and debug payloads out of serializable UI state.
- T-011-004 adds a dependency-free setup-screen contract (`src/ui/setup-screens.js`) for Capture Setup, OCR Preview, and Translation Settings. It emits safe API intent descriptors for source enumeration, allow-listed profile updates, OCR preview, write-only provider key save, and translation preview while keeping API keys and manual translation text fetch-time-only. OCR/translation preview text is permitted only in transient visible screen state and is redacted from log-safe snapshots, sanitized errors, diagnostics-facing descriptors, and any durable state.
- T-011-005 adds a dependency-free closeout-screen contract (`src/ui/closeout-screens.js`) for Overlay Theme Editor, OBS Setup Guide, Privacy Settings, and Logs/Diagnostics. It emits safe intents for existing theme, profile theme, privacy, diagnostics, status/WebSocket, clipboard, and host-command surfaces; keeps built-in themes read-only; trusts only sanitized localhost overlay URLs; shows privacy warnings before debug persistence; and keeps diagnostic logs, debug paths, provider keys, OCR/source text, translated text, screenshots, stack traces, and provider responses out of log-safe snapshots.
- T-012-001 adds a dependency-free synthetic first-run stream harness (`src/core/synthetic-first-run-stream.js`) that drives a frozen Japanese OCR candidate through the existing OCR-to-overlay runtime pipeline with an injected deterministic provider and `OverlayState`. It returns a frozen privacy-safe evidence summary proving an overlay subtitle was published within the 5-minute first-run budget, without adding routes, native dependencies, persistence, provider keys, source-text logs, translated-text logs, images, screenshots, or debug payloads.
- T-012-002 adds `npm run smoke:first-run-stream` as the live localhost first-run stream smoke. The command starts the API/overlay server on an ephemeral `127.0.0.1` port, shares an in-memory `OverlayState` with the synthetic first-run harness, proves `/health`, `/api/status`, `/overlay`, `/ws/app`, and `/ws/overlay` carry the translated subtitle through live and replay paths, and prints only hash/id/count evidence with no raw Japanese source text, translated text, provider keys, screenshots, images, or logs.
- T-012-003 adds `npm run smoke:backend-recovery` as the backend restart and preferred-port conflict smoke. The command proves a fresh localhost backend can start on a selected port, stop, restart on the same preferred port with overlay HTTP/WebSocket replay intact, fall back to a later localhost port when the preferred port is occupied, report retryable `PORT_UNAVAILABLE` when attempts are exhausted, and reclaim the preferred port after the conflict clears. Its stdout is limited to ids, ports, error codes, retryability flags, hashes, and named checks.
- T-012-004 adds `npm run smoke:overlay-layout` as the OBS overlay resolution layout smoke. The command renders the existing overlay HTML contract for `classic_subtitle`, `stream_box`, and `minimal` across max line counts 1-3 and OBS Browser Source sizes 1280x720, 1920x1080, and 2560x1440. It verifies transparent fixed full-viewport layout, safe-area padding, max-width/font-size clamps, line-height, line clamp, emergency wrapping, no external assets, and conservative no-clipping/no-overlap layout estimates while printing only hash/count/resolution evidence.
- T-012-005 adds `npm run smoke:first-run-closeout` plus `FIRST_RUN_STREAM_CLOSEOUT_RUNBOOK_JA.md` as the parent closeout evidence path for the first-run stream core. The closeout command runs the T-012 first-run, backend-recovery, and overlay-layout smoke modules in sequence, scans their canonical evidence summaries for known raw text/secret/path/stack/debug leak sentinels, and prints only child command ids, module execution mode, exit-code equivalents, durations, summary byte counts, SHA-256 hashes, schema versions, runbook/package check counts, and the explicit `WINDOWS_OBS_VISUAL_GATE` manual-gate marker. It does not claim real Windows/OBS visual validation; that remains a human milestone gate.
- T-013-001 adds the dependency-free desktop host lifecycle contract (`src/desktop/host-lifecycle.js`) for the future Electron main process. It normalizes a localhost-only API server config, starts/stops/restarts an injected `createLocalApiServer`-compatible backend, exposes frozen host snapshots with trusted `http://127.0.0.1:<port>/overlay` and `/ws/app` URLs, maps startup failures to sanitized retryable/non-retryable errors, and executes the existing Home/Status `restart_backend` host-command intent without accepting sensitive or prototype-inherited commands.
- T-013-002 adds the dependency-free desktop IPC bridge contract (`src/desktop/ipc-bridge.js`) for the future Electron preload/main boundary. It accepts only the renderer intent descriptors emitted by existing UI contracts, routes safe HTTP/WebSocket/clipboard/navigation/host-command intents through injected adapters, validates localhost-only overlay/status URLs and API paths, binds `restart_backend` to the T-013-001 lifecycle, redacts log-safe records, and rejects unsupported, inherited, sensitive, remote, filesystem, or arbitrary command payloads.
...
- [x] T-011 First-Run flow has an executable renderer contract for profile/key/capture/OCR/translation/activation sequencing, with API key and raw translation text redaction locked by unit tests.
- [x] T-011 Home/Status has an executable renderer contract for trusted OBS URL copy feedback, status fetch/stream connection intents, code-driven recovery actions, and capture start/stop intents.
- [x] T-011 Capture/OCR/Translation setup screens have an executable renderer contract for source enumeration, profile setting updates, OCR preview, provider key save, translation preview, transient preview redaction, and code-driven recovery actions.
- [x] T-011 Overlay/Privacy/Diagnostics closeout screens have an executable renderer contract for theme CRUD/apply, OBS guide trusted URL actions, privacy warning/update flows, diagnostics bundle fetch/copy, redaction, and code-driven recovery actions.
- [x] T-012-001 synthetic first-run stream harness proves a Japanese OCR fixture can publish an English overlay subtitle through the runtime pipeline within the 5-minute budget while returning only privacy-safe evidence hashes.
- [x] T-012-002 first-run stream smoke proves the synthetic Japanese fixture reaches the localhost OBS overlay HTTP and WebSocket surfaces within the 5-minute budget while stdout remains privacy-safe hash evidence.
- [x] T-012-003 backend recovery smoke proves localhost backend restart, preferred-port fallback, retryable port exhaustion, and overlay HTTP/WebSocket replay recovery while stdout remains privacy-safe hash/code evidence.
- [x] T-012-004 overlay layout smoke verifies built-in overlay themes and 1-3 line subtitle layouts stay inside safe areas without clipping or overlap at 1280x720, 1920x1080, and 2560x1440 while stdout remains privacy-safe hash/count evidence.
- [x] T-012-005 first-run stream closeout runbook aggregates the T-012 smoke evidence, validates required runbook/package commands, keeps stdout privacy-safe, and records that real Windows/OBS visual validation remains a separate manual gate.
- [x] T-013-001 desktop host lifecycle contract starts, stops, restarts, and reports the localhost backend through frozen privacy-safe snapshots, rejects non-localhost/sensitive host commands, and wires Home/Status `restart_backend` to the host lifecycle without adding Electron/native dependencies.
- [x] T-013-002 desktop IPC bridge contract dispatches existing renderer intents through allow-listed host adapters, preserves localhost-only and trusted-overlay gates, redacts log-safe records, and blocks unsupported or sensitive host payloads without adding Electron/native dependencies.
