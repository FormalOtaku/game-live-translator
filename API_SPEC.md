...
## Desktop Host Lifecycle Contract
- T-013-001 adds `src/desktop/host-lifecycle.js`, a dependency-free pure JS contract for the future Electron main process. It owns backend process lifecycle semantics without importing Electron, native capture APIs, OCR engines, provider clients, SQLite drivers, or OS keychain APIs.
- Config: `normalizeDesktopHostConfig({ bindAddress?, preferredPort?, maxPortAttempts?, version?, appWsPath? })` always requires `bindAddress === "127.0.0.1"`, defaults to the existing local API server port/version/path constants, allows `preferredPort=0` for ephemeral test ports, and rejects invalid port attempts or non-absolute slash-separated WebSocket paths before startup. Prototype-inherited config fields are ignored/rejected through plain-object and own-property checks.
- Lifecycle: `createDesktopHostLifecycle({ config?, apiFactory?, serverOptions?, clock? })` injects a `createLocalApiServer`-compatible adapter with `start()` and `stop()`. `start()` returns a frozen snapshot with `state: "ready"`, `backendState: "ready"`, selected port, trusted `overlayUrl`, trusted `appWsUrl`, generation count, timestamps, and privacy guarantees. `stop()` clears port/URL metadata and returns `state: "stopped"` with renderer-safe `backendState: "degraded"` so recovery UI does not show an indefinite loading state. Invalid adapter shapes, invalid started ports, unavailable ports, or adapter throws return `state: "error"` snapshots with sanitized `lastError`; partial startup failures and failed-stop recovery retries must call adapter `stop()` for cleanup before a new adapter is created.
- Restart and commands: `restart()` stops the current adapter when present and starts a fresh one, incrementing `restartCount` and `generation`. `executeHostCommand(intent)` accepts only own-property `{ type: "hostCommand", command: "restart_backend", sensitive?: false }` intents from the renderer contract. Sensitive, unsupported, inherited, or malformed commands fail with controlled `ContractError` codes and must not echo raw command text.
- Snapshot privacy: host snapshots may contain only state, localhost bind/port/URL metadata, timestamps, restart/generation counts, controlled error codes/messages/retryability, value-free safe details, and privacy booleans. They must not serialize provider keys, raw OCR/source text, translated text, screenshots or image paths, stack traces, raw logs, provider responses, game file paths, or debug payloads. The contract adds no HTTP routes; it wraps the existing localhost API server contract.

## Desktop IPC Bridge Contract
- T-013-002 adds `src/desktop/ipc-bridge.js`, a dependency-free pure JS contract for the future Electron preload/main IPC boundary. It imports no Electron, native capture APIs, OCR engines, provider clients, SQLite drivers, OS keychain APIs, filesystem APIs, or shell process APIs; concrete side effects are supplied through injected adapters.
- Channel and payload gate: `createDesktopIpcBridge({ lifecycle, adapters?, logger? })` exposes `dispatch(channel, intent)`, `safeIntentForLog(intent)`, and `describe()`. The only allowed channel is `"gameLiveTranslator:intent"`. Intents must be own-property plain objects; prototype-inherited, class instance, array, null, function, unknown-type, and unknown-command payloads are rejected with controlled `IpcResult.error` objects.
- Supported intent types match the renderer contracts: `"http"`, `"websocket"`, `"clipboard.writeText"`, `"navigate"`, and `"hostCommand"`. HTTP intents accept only allow-listed methods and local API paths beginning with `/api/` or `/health`; sensitive HTTP intents are rejected, and accepted requests forward only method/path/body metadata to the injected HTTP adapter and never serialize adapter response bodies. WebSocket intents require `ws://127.0.0.1:<port>/ws/app`; clipboard intents require non-empty text and redact text from bridge results/logs; navigate intents are route-only and perform no host side effect.
- Host commands are allow-listed: `restart_backend` calls the T-013-001 lifecycle, `open_url` may open only trusted `http://127.0.0.1:<port>/overlay` through the injected browser adapter, `open_debug_folder` and `clear_debug_data` call the injected privacy adapter with command ids only, and troubleshooting/wait/retry commands return controlled acknowledgements. Host commands must not carry filesystem paths, shell commands, raw URLs other than trusted overlay open, provider keys, OCR/source text, translated text, screenshots, stack traces, raw logs, or debug payloads.
- Result shape: `dispatch(...)` returns frozen `{ schemaVersion: "desktop-ipc-bridge.v1", ok, channel, intentType, actionId, result, error, privacy }`. Successful results contain only status/acknowledgement metadata such as `status`, `connected`, `written`, `route`, `command`, or lifecycle `state`; they never echo clipboard text, HTTP bodies/responses, diagnostics logs, debug directories, adapter internals, or raw lifecycle errors. Failures use controlled codes/messages and value-free safe details.
- Logging: the bridge calls the optional logger with `safeIntentForLog(intent)` only. Log-safe records keep action id, type, route, method/path, command id, URL trust labels, body key names, and booleans/counts. They must not include provider keys, OCR/source text, translated text, clipboard contents, diagnostics bundle text, screenshots, stack traces, raw logs, debug directories, shell commands, arbitrary command payloads, or untrusted URLs.
