...
- Route normalization: `normalizeRoute(routeId, { setup? })` accepts only known first-class ids, redirects `requiresSetup` routes to `first-run` when setup is incomplete, and falls back to `home` for unknown ids, prototype-pollution-shaped strings, non-strings, and `null`/`undefined`. Unknown user-supplied route ids never echo through the resolved route.
