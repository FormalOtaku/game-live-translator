...
- Verification evidence: T-012-001 adds a non-UI synthetic first-run stream harness that proves the runtime pipeline can publish an English overlay subtitle from a Japanese fixture within the same 5-minute setup budget. T-012-002 wraps that harness in `npm run smoke:first-run-stream`, which proves the same fixture reaches the localhost OBS overlay HTTP and WebSocket surfaces. T-012-003 adds `npm run smoke:backend-recovery`, which proves backend restart, preferred-port fallback, retryable port exhaustion, and overlay HTTP/WebSocket replay recovery. T-012-004 adds `npm run smoke:overlay-layout`, which proves the built-in OBS overlay layout contract stays within safe areas for 1280x720, 1920x1080, and 2560x1440 Browser Source sizes. T-012-005 adds `npm run smoke:first-run-closeout`, which aggregates the T-012 smoke evidence into privacy-safe hash/count/schema records and explicitly marks real Windows/OBS visual confirmation as a separate manual gate. The First-Run UI may display only sanitized pass/fail evidence from those harnesses or smoke summaries; it must not display or log the synthetic source text, translated text, escaped text, provider keys, images, screenshots, raw child logs, cache keys, stack traces, or debug payloads from the evidence summaries.
...
- Host lifecycle: T-013-001 binds the Home/Status `restart_backend` host-command intent to the desktop host lifecycle contract. The host may restart only a localhost `127.0.0.1` backend adapter, must reject sensitive or inherited host-command fields, and must return sanitized lifecycle snapshots/error states without showing provider keys, raw OCR/source text, translated text, screenshots, stack traces, raw logs, or debug payloads.
...
- Host binding: `src/desktop/host-lifecycle.js` is the first executable host-side consumer of the `restart_backend` intent. It normalizes the selected backend config, restarts the injected API adapter, reports the trusted overlay URL from the selected localhost port, and sanitizes lifecycle failures for Home/Status recovery banners.
- IPC binding: T-013-002 adds `src/desktop/ipc-bridge.js` as the safe host-side dispatcher for renderer intents. The future Electron preload must send only the existing frozen intent descriptors over the single allow-listed channel, and the main process must bind them through this bridge so HTTP, WebSocket, clipboard, route navigation, browser-open, backend restart, troubleshooting, and privacy debug actions keep the same localhost and redaction gates.
